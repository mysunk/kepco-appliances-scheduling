...
class Appliance():
    ...
    def __init__(self, action_size=1):
        self.action_set = list(range(action_size+1))
        # 1시간에 scale [kwh] 만큼 사용하는 appliance
        # self.scale = 0.15
        self.scale = 0.5
        self.user_hour = 20
        self.user_tol = 72 # 72시간동안은 최대한 쓰면 안 됨
    ...
    def compute(self,state,action):
        ...
        ts_stack = state[2]
        hour = state[3] * 24
        current_load = self.action_set[action] * self.scale
        ...
        if action:
            ...
            if ts_stack < 1:
                ...
                # 사용 안할 시 시간이 지날수록 패널티..
                penalty1 = -ts_stack**2 * 5
            else:
                # 3일 ~ 6일 안에는 세탁기를 돌린다..
                penalty1 = 5

            # (평균에 비한 가격) * 사용량 만큼 패널티 또는 리워드 받음
            # reward1 = (smp - avg_price) * self.scale
            if smp > avg_price:
                reward1 = -3
            else:
                reward1 = +3
            ...
            # 시간에 따른 패널티
            penalty2 = -0.1 * np.minimum(np.abs(hour - self.user_hour), np.abs(hour + 24 - self.user_hour))**2

            reward = reward1 + penalty1 + penalty2

            # next stack state
            ts_stack = 0
        else:
            ts_stack += 1 / self.user_tol
            reward = 0

        return ts_stack, current_load, reward
...
class Battery():
    ...
    def __init__(self, action_size, scaler_energy, scaler_price):
        ...
        self.action_set = np.concatenate(
            (self.action_sell, self.action_use, np.array([0])))  # 아무것도 안하는거/방출하는거(판매,자가사용))
        self.initial_SOC = 0.
    ...
    def compute(self, state, action, timestep):
        ...
        tax_cost = Tax_fn_sparse(acc_load, timestep+1, self.scaler_energy, self.scaler_price)
        # print(f'{timestep}, {acc_load}, {tax_cost}')
        consumed_price = 0
        ...
        if action < 4:  # 판매했을경우만 거래비용 고려
            ...
            trading_price = self.scaler_price.inverse_transform(np.array([current_price]).reshape(-1, 1)) \
                            * self.scaler_energy.inverse_transform(
                np.array([-1 * delta_SOC * self.E_rated]).reshape(-1, 1))  # 실제로 판 가격
...
def Memory_Initialization(memory, battery, timesteps, pretrain_length, x, action_size, sc_energy):
    ...
    for i in range(pretrain_length):
        ...
        accumulated_load = np.cumsum(load_list, axis=0)
        ...
        if hour < 23:
            ...
            historical_price[timestep] = x[day * 24 + hour, 2]
            average_price = np.mean(np.array([price for price in historical_price if price != 0]))
            next_state = np.concatenate((x[day * 24 + hour, :], next_SOC, np.array([average_price]), np.array([accumulated_load[day*24]])), axis=-1)
        else:
            done = True
            day += 1
            hour = 0
    ...
    for i in range(pretrain_length):
        ...
        if hour < 23:
            ...
            ts_next_state = np.concatenate(
                (x[day * 24 + hour + 1, 2:], np.array([average_price]), np.array([ts_stack]), np.array([hour / 24])),
                axis=-1)
        else:
            done = True
            day += 1
...
# def Tax_fn_sparse(acc_load, sc_energy, sc_price, test_duration=24 * 28):
#
...
def Tax_fn_sparse(load, timestep, sc_energy, sc_price): #unit[kW/h]
    day = (timestep+1) // 24
    if day == 0:
        load = 0
    else:
    ...
    thres_q1 = sc_energy.transform(np.array([200]).reshape(-1,1))[0,0] #[W/day]
    thres_q2 = sc_energy.transform(np.array([400]).reshape(-1,1))[0,0]
    applied_price = [sc_price.transform(np.array([data]).reshape(-1, 1))[0, 0] for data in [93.3, 187.9, 280.6]]
    # price_sum = 0
    applied_price_sum = 0
    if load < thres_q1:
        # price_sum += inverse_transform(sc_energy, load) * inverse_transform(sc_price, 93.3)
        applied_price_sum += load * applied_price[0]
    elif load < thres_q1 + thres_q2:
        # price_sum += inverse_transform(sc_energy, thres_q1) * inverse_transform(sc_price, 93.3)
        # price_sum += inverse_transform(sc_energy, (load - thres_q1)) * inverse_transform(sc_price, 187.9)
        applied_price_sum += thres_q1 * applied_price[0]
        applied_price_sum += (load - thres_q1) * applied_price[1]
    else:
        # price_sum += inverse_transform(sc_energy, thres_q1) * inverse_transform(sc_price, 93.3)
        # price_sum += inverse_transform(sc_energy, (load - thres_q1)) * inverse_transform(sc_price, 187.9)
        # price_sum += inverse_transform(sc_energy, (load - thres_q1 - thres_q2)) * inverse_transform(sc_price, 280.6)
        applied_price_sum += thres_q1 * applied_price[0]
        applied_price_sum += (load - thres_q1) * applied_price[1]
        applied_price_sum += (load - thres_q1 - thres_q2) * applied_price[2]
    reward = applied_price_sum / 28
    return reward
